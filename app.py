...
def analyze_endpoint():
    try:
        ...
        # === LÓGICA OPTIMIZADA ===
        # 1. Creamos todas las tareas SERP primero
        tasks_data = []
        for keyword in keywords:
            tasks_data.append({"keyword": keyword, "language_name": "Spanish", "location_code": 2724, "depth": 20})
        ...
        task_ids_to_get = [{"id": task['id']} for task in posted_tasks if task.get('status_code') == 20100]
        if not task_ids_to_get:
            return jsonify({"error": "Ninguna tarea SERP se pudo crear correctamente."}), 500
            
        serp_results_tasks = post_to_dataforseo("serp/google/organic/task_get/advanced", task_ids_to_get)['tasks']

        # Creamos un diccionario para acceder fácilmente a los resultados por keyword
        results_by_keyword = {task['data']['keyword']: task.get('result') for task in serp_results_tasks}
        ...
        for keyword in keywords:
            ...
            if not serp_results:
                keyword_analyses.append({"keyword": keyword, "error": "No se pudo obtener el resultado de la tarea SERP."}); continue
